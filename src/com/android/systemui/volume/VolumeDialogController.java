...
public class VolumeDialogController {
    ...
    public static final String BROADCAST_SHOW_VOLUME_BAR = "com.android.systemui.volume.show_volumebar";
...
//        mWorker.obtainMessage(W.SET_STREAM_VOLUME, stream, level).sendToTarget();
    ...
    private void onNotifyVisibleW(boolean visible) {
        FlyLog.d("onNotifyVisibleW visible=" + visible);
        if (mDestroyed) return;
...
//        }
    ...
    private boolean updateActiveStreamW(int activeStream) {
        FlyLog.e("updateActiveStreamW activeStream=%d,mState.activeStream=%d", activeStream, mState.activeStream);
...
//            }
    ...
    private static boolean isLogWorthy(int stream) {
        switch (stream) {
            ...
            case AudioSystem.STREAM_GIS:
            case AudioSystem.STREAM_AUXIN:
                return true;
        }
        return false;
    }
    ...
    private void onSetActiveStreamW(int stream) {
        FlyLog.d("onSetActiveStreamW stream=%d", stream);
    ...
    private final class VC extends IVolumeController.Stub {
        ...
        public void volumeChanged(int streamType, int flags) throws RemoteException {
            ...
            int lastvalue = loadLastVolume(streamType);
            FlyLog.e("lastSaveVolume: " + lastvalue);
            if (flags == 4113) {
                if (currentVolume == 0) {
            ...
            if (flags == 4113) {
                saveLastVolume(streamType, currentStream);
            }
...
//                }
    ...
    private final class MediaSessionsCallbacks implements MediaSessions.Callbacks {
        ...
        public void onRemoteVolumeChanged(Token token, int flags) {
            FlyLog.d("onRemoteVolumeChanged token=" + token + ",flags=" + flags);
