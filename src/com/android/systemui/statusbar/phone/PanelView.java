...
public abstract class PanelView extends FrameLayout {
	...
	private boolean mActionDownFlag = false;
    ...
    private void schedulePeek() {
		...
		if (DEBUG) logf("schedulePeek timeout=%d", timeout);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
				mActionDownFlag = true;
				mInitialTouchY = y;
				if (DEBUG) logf(" onTouchEvent mInitialTouchY(%.1f)", mInitialTouchY);
            ...
            case MotionEvent.ACTION_MOVE:
				if (DEBUG) logf("onTouchEvent");
				if(mActionDownFlag){
					float h = y - mInitialTouchY;
					if (DEBUG) logf("move height=%.1f", h);
					if (DEBUG) logf("mTouchSlop=%d", mTouchSlop);
					if (DEBUG) logf("mIgnoreXTouchSlop=%b", mIgnoreXTouchSlop);
					// If the panel was collapsed when touching, we only need to check for the
					// y-component of the gesture, as we have no conflicting horizontal gesture.
					if (Math.abs(h) > mTouchSlop
							&& (Math.abs(h) > Math.abs(x - mInitialTouchX)
									|| mIgnoreXTouchSlop)) {
					mActionDownFlag = false;
    ...
    if (DEBUG) logf("onInterceptTouchEvent");
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                mInitialTouchY = y;
				if (DEBUG) logf("mInitialTouchY(%.1f)", mInitialTouchY);
                mInitialTouchX = x;
