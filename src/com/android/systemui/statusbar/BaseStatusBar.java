...
public abstract class BaseStatusBar extends SystemUI implements
    ...
    public boolean isDeviceProvisioned() {
        ...
        if (mDeviceProvisioned == false) {
            ...
            Log.d(TAG, "mDeviceProvisioned is true");
        ...
        public void handleMessage(Message m) {
            switch (m.what) {
             ...
             case MSG_TOGGLE_FLOAT_PANEL:
                 Log.d(TAG, "Toggle float panel.");
                 toggleFloatPanel();
                 break;
             case MSG_CLOSE_FLOAT_PANEL:
                 Log.d(TAG, "Close float panel.");
                 closeFloatPanel();
                 break;
              /// @}
            }
    ...
    protected boolean shouldInterrupt(Entry entry, StatusBarNotification sbn) {
        ...
        if (isSnoozedPackage(sbn)) {
            /// M: Add the log for snoozed package notification.
            Log.d(TAG, "Snoozed package notification: "
                    + (sbn != null ? sbn.getNotification() : ""));
            return false;
        }
        ...
        if (true/*DEBUG*/) Log.d(TAG, "interrupt: " + interrupt);
        return interrupt;
    }
    ...
    public void toggleFloatPanel() {
        ...
        Log.d(TAG, "toggleFloatPanel: mFloatPanelView = " + mFloatPanelView + ", shown = "
        ...
        if (floatPanelOpen) {
            ...
            if(mNavigationBarView != null){
               mNavigationBarView.refreshRestoreButton();
            }
        } else {
            WindowManager.LayoutParams lp = getFloatLayoutParams(mFloatPanelView.getLayoutParams(),
                    true);
    ...
    public void toggleFloatPanelScreenOff() {
        /// M: ALPS01898743,ALPS01901240,always show float button when
        /// keyguard not set / Don not show float button on keyguard
        Log.d(TAG, "toggleFloatPanelScreenOff");
        //for M: ALPS02056031
        mSecurityModel = new KeyguardSecurityModel(mContext);
        SecurityMode model =  mSecurityModel.getSecurityMode();
        if(model == SecurityMode.None){
            closeFloatPanel();
        }else{
    ...
    public void closeFloatPanel() {
        ...
        Log.d(TAG, "closeFloatPanel: mFloatPanelView = " + mFloatPanelView + ",visible = "
                + visible + ", shown = "
     ...
     public void closeFloatPanelNotShowFloatButton() {
          ...
          final int  visible =  View.STATUS_BAR_DISABLE_HOME
                ...
                if(mIsSplitModeEnable){
                    setFloatModeButtonVisibility(View.INVISIBLE);
                    setSplitModeButtonVisibility(View.INVISIBLE);
                    setLineVisibility(View.INVISIBLE);
                }
                mWindowManager.removeView(mFloatPanelView);
                mFloatPanelView = null;
                }else{
